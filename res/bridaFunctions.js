...
function getallmodules() {
    let results = {}
    let matches = Process.enumerateModules({
...
function getmoduleimports(importname) {
    let results = {}
    let matches = Module.enumerateImports(importname, {
        onMatch: function (module) {
...
function getmoduleexports(exportname) {
    let results = {}
    let matches = Module.enumerateExports(exportname, {
        onMatch: function (module) {
            results[module['type'] + ": " + module['name']] = module['address'];
        },
        onComplete: function () {
...
function getclassmethods(classname) {
    ...
    if (ObjC.available) {
        ...
        let matches = resolver.enumerateMatches("*[" + classname + " *]", {
...
function findobjcmethods(searchstring) {
    let results = {}
    let resolver = new ApiResolver("objc");
    let matches = resolver.enumerateMatches("*[*" + searchstring + "* *]", {
        onMatch: function (match) {
            results[match['name']] = match['address'];
        },
        onComplete: function () {
...
function findimports(searchstring) {
    ...
    let matches = resolver.enumerateMatches("imports:*" + searchstring + "*!*", {
        onMatch: function (match) {
            results[match['name']] = match['address'];
        },
        onComplete: function () {
...
function findexports(searchstring) {
    let results = {}
    let resolver = new ApiResolver("module");
    let matches = resolver.enumerateMatches("exports:*" + searchstring + "*!*", {
        onMatch: function (match) {
            results[match['name']] = match['address'];
        },
        onComplete: function () {
        ...
        targets.forEach(function (target) {
            Interceptor.attach(target.address, {
                ...
                onLeave: function (retval) {
                    ...
                    if (typeret === "String") {
                        let stringClass = Java.use("java.lang.String");
                        let toRet = stringClass.$new(newret);
                    } else if (typeret === "ptr") {
                        toRet = ptr(newret);
                    } else if (typeret === "Boolean") {
                        if (newret === "true") {
